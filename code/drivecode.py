...
testimg[:, :, 0] = patchify.unpatchify(
    np.asarray([clusterGtR[j] for i, j in enumerate(t25redPred)]), (584, 565))
testimg[:, :, 1] = patchify.unpatchify(
    np.asarray([clusterGtG[j] for i, j in enumerate(t25greenPred)]), (584, 565))
testimg[:, :, 2] = patchify.unpatchify(
    np.asarray([clusterGtB[j] for i, j in enumerate(t25bluePred)]), (584, 565))
